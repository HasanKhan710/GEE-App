...
#                 st.plotly_chart(fig_bus, use_container_width=True)
    ...
    planned_slack_per_hour = st.session_state.network_data["df_gen_profile"]["p_mw_slack"].tolist()
    slack_per_hour_bau     = st.session_state.bau_results["slack_per_hour_bau"]
    slack_per_hour_wa      = st.session_state.weather_aware_results["slack_per_hour"]

    # Get generator data
    df_gen_params = st.session_state.network_data["df_gen"]
    gen_per_hour_bau = st.session_state.bau_results["gen_per_hour_bau"]
    gen_per_hour_wa = st.session_state.weather_aware_results["gen_per_hour"]
    ...
    # persistent UI flags
    # --------------------------------------------------------------------- #
    for k in ("show_comp", "show_diff", "show_lines", "show_bus", "show_slack", "show_gen", "gen_to_plot"):
        if k not in st.session_state:
            st.session_state[k] = False if k != "gen_to_plot" and k != "bus_to_plot" else None
    ...
    with col3:
        if st.button("Line‑Loading‑over‑Time"):
            st.session_state.show_lines = True
    with col4:
        if st.button("Hourly Slack Generator Dispatch"):
            st.session_state.show_slack = True
    with col5:
        if st.button("Generator Dispatch Comparison"):
            st.session_state.show_gen = True
            
    # ------------------------------------------------------------------ #
    # Extra UI for "Load Bus Plot" – pick a load‑bus & make the button
    # ------------------------------------------------------------------ #
    ...
    # ------------------------------------------------------------------ #
    # Extra UI for "Generator Plot" – pick a generator & make the button
    # ------------------------------------------------------------------ #
    with col2:
        gen_options = df_gen_params["bus"].astype(int).tolist()[1:]  # Skip slack gen
        sel_gen = st.selectbox("Select a Generator for detailed dispatch comparison:", 
                              gen_options, key="gen_select")
    ...
    # ===================================================================== #
    # PLOT 6  ─ Hourly Generator Dispatch at specific Generator
    # ===================================================================== #
    if st.session_state.show_gen and st.session_state.gen_to_plot is not None:
        gen = st.session_state.gen_to_plot
        
        # Find index of the generator in the generator parameters dataframe
        try:
            gen_df = df_gen_params
            gen_idx = gen_df.index[gen_df["bus"] == gen].tolist()[0] - 1  # Adjusting index (skip slack)
            
            # Get the generator profile data
            gen_profile_df = st.session_state.network_data["df_gen_profile"]
            gen_col = f"p_mw_PV{gen}"
            
            if gen_col in gen_profile_df.columns:
                original_gen = gen_profile_df[gen_col].tolist()
                served_gen_bau = [hour[gen_idx] for hour in gen_per_hour_bau]
                served_gen_wa = [hour[gen_idx] for hour in gen_per_hour_wa]
                ...
                fig_gen.update_layout(
                    title=f"Comparison of Hourly Generator Dispatch at Generator {gen}",
                    xaxis=dict(title="Time [in Hours]", tickmode='linear'),
                    yaxis=dict(title="Generation (MWh)"),
                    barmode='group',
                    template='plotly_dark',
                    font=dict(family="Arial", size=14),
                    legend=dict(title='Generation Type'),
                    margin=dict(l=40, r=40, t=60, b=40),
                    height=600,
                    width=1200
                )
            ...
            st.warning(f"Generator {gen} not found in Generator Parameters – cannot plot.")
