...
#                 st.plotly_chart(fig_bus, use_container_width=True)
    ...
    # persistent UI flags
    # --------------------------------------------------------------------- #
    for k in ("show_comp", "show_diff", "show_lines", "show_bus", "bus_to_plot", "show_slack_comp", "show_gen_comp"):
        if k not in st.session_state:
            st.session_state[k] = False if k != "bus_to_plot" else None
    ...
    # --------------------------------------------------------------------- #
    # New buttons for Slack Generator Dispatch Comparison and Generator Dispatch Comparison
    # --------------------------------------------------------------------- #
    col4, col5 = st.columns(2)
    with col4:
        if st.button("Hourly Slack Generator Dispatch Comparison"):
            st.session_state.show_slack_comp = True
    with col5:
        if st.button("Hourly Generator Dispatch at Generator"):
            st.session_state.show_gen_comp = True

    # ===================================================================== #
    # PLOT 1 ─ Hourly Slack Generator Dispatch Comparison
    # ===================================================================== #
    if st.session_state.show_slack_comp:
        # Sample data (replace with your actual data)
        hours = list(range(24))
        original_load = st.session_state.weather_aware_results["planned_slack_per_hour"]
        served_opf = st.session_state.bau_results["slack_per_hour_bau"]
        served_opf_wa = st.session_state.weather_aware_results["slack_per_hour_wa"]
    ...
    # ===================================================================== #
    # PLOT 2 ─ Hourly Generator Dispatch Comparison at Generator
    # ===================================================================== #
    if st.session_state.show_gen_comp:
        # Drop-down menu for generator selection
        df_load_params = pd.read_excel("path_to_your_file.xlsx", sheet_name="Generator Parameters", index_col=0)
        gens = df_load_params["bus"].tolist()[1:]  # Exclude the first row which is header

        selected_gen = st.selectbox("Select Generator for Comparison:", gens)

        # Get generator data
        bus_col = f"p_mw_PV{selected_gen}"

        if bus_col not in df_load_profile.columns:
            st.warning(f"Column {bus_col} not found in Generator Profile.")
        else:
            hours = list(range(24))
            original_load = df_load_profile[bus_col].tolist()
            served_opf = [hour[i] for hour in st.session_state.bau_results["gen_per_hour_bau"]]
            served_opf_wa = [hour[i] for hour in st.session_state.weather_aware_results["gen_per_hour_wa"]]
            ...
            fig_gen.update_layout(
                title=f"Hourly Generator Dispatch Comparison at Generator {selected_gen}",
                xaxis=dict(title="Time [in Hour]", tickmode='linear'),
